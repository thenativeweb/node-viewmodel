...
Repository.use = function (toRequire) {
  ...
  if (toRequire.indexOf('.') !== 0 && toRequire.indexOf('/') !== 0 && toRequire.split('/').length < 2) {
    try {
      var requiredPkgJson;
      requiredPkgJson = prequired ? prequire(toRequire + '/package.json') : require(toRequire + '/package.json');
      if (requiredPkgJson && requiredPkgJson.engines && requiredPkgJson.engines.node &&
          !semver.satisfies(process.version, requiredPkgJson.engines.node)) {
        console.error('Your node.js runtime version (' + process.version + ') does not satisfy the version requested by ' + toRequire + ' (' + requiredPkgJson.engines.node + ')!');
        process.exit(1);
      }
    } catch (e) {}
  }
