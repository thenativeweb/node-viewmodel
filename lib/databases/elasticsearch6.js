...
_.extend(Elasticsearch.prototype, {
  connect: function(callback) {
    ...
    this.client = new elasticsearch.Client({
      host: (options.port.indexOf(':') > -1) ? options.host : options.host + ":" + options.port,
      log: options.log
    });
    ...
    this.client.indices.create(
      ...
      function(err) {
        ...
        if (err) {
          ...
          var i = this.indexAndTypeNames.indexOf(this.indexAndTypeName);
          if (i > -1) {
            this.indexAndTypeNames.splice(i,1);
          }
          return callback(err);
        }
  ...
  clearAll: function(callback) {
    ...
    if (!self.indexAndTypeNames || !slef.indexAndTypeNames.length) callback(null);

    const self = this;

    async.each(self.indexAndTypeNames, _clearIndex, callback);
  },
