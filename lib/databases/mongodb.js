...
var ViewModel = Repository.ViewModel;
var ConcurrencyError = require('../concurrencyError');
var mongo = Repository.use('mongodb');
var mongoVersion = Repository.use('mongodb/package.json').version;
var isNew = mongoVersion.indexOf('1.') !== 0;
var ObjectID = isNew ? mongo.ObjectID : mongo.BSONPure.ObjectID;
var _ = require('lodash');
var async = require('async');
var collections = [];
var nodeNativeHelpers = require('node-native-helpers');
var MeasurePerformance = nodeNativeHelpers.MeasurePerformance;
var MeasurePerformanceEvent = nodeNativeHelpers.MeasurePerformanceEvent;
...
_.extend(Mongo.prototype, {
  ...
  commit: function (vm, callback) {
    var performance = new MeasurePerformance(vm.id);

    var self = this;
    performance.on(MeasurePerformanceEvent.Measure, function (duration) {
      self.emit(MeasurePerformanceEvent.Measure, { vm: vm, duration: duration })
    });
