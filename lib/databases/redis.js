      ...
      case 'delete':
        ...
        this.client.watch(prefixedId, function(err) {
          ...
          self.get(vm.id, function(err, savedVm) {
            ...
            self.emit('database-redis-commit', vm);
            self.client.multi([['del', prefixedId]]).exec(function(err, replies) {
              self.emit('after-redis-commit', vm);
              if (err) {
                return callback(err);
              }
      ...
      case 'create':
        this.client.get(prefixedId, function(err, data) {
          ...
          self.get(vm.id, function(err, savedVm) {
            ...
            self.emit('before-database-commit', { vm: vm, emitTime: (new Date()).getTime() });
            self.client.set(prefixedId, obj, function(err) {
              self.emit('after-database-commit', { vm: vm, emitTime: (new Date()).getTime() });
              vm.actionOnCommit = 'update';
              callback(err, vm);
            });
          });
