...
_.extend(Elasticsearch.prototype, {
        ...
        case "delete":
          ...
          self.client.delete(
            ...
            function(err, res) {
              if (
                err &&
                (err.message.toLowerCase().indexOf("version") >= 0 ||
          ...
          break;
        case "create":
          self._insert(vm, callback);
          break;
        case "update":
          // version 0 means new / unsaved viewmodel
          if (vm.version === 0) {
            self._insert(vm, callback);
            return;
          }
          ...
          self.client.index(
            ...
            function(err, res) {
              if (
                err &&
                (err.message.toLowerCase().indexOf("version") >= 0 ||
                  err.message.toLowerCase().indexOf("already") >= 0)
              ) {
                return callback(new ConcurrencyError());
              }
              vm.actionOnCommit = "update";
  ...
  clear: function(callback) {
    ...
    this.checkConnection(function(err) {
      ...
      self._clearCollection(self.collectionName, callback);
    });
  },
  ...
  clearAll: function(callback) {
    ...
    const self = this;
    async.each(collections, self._clearCollection, callback);
  },

  // private methods
  _clearCollection: function(col, callback) {
    const self = this;
